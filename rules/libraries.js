...
const testRules = {
  GWT: {
    ...
    test(win) {
      ...
      const hasHistFrame = doc.getElementById('__gwt_historyFrame');
      const hasGwtUid = doc.gwt_uid;
      const hasBodyListener = doc.body.__listener;
      ...
      if (
        ...
        let gwtVersion = UNKNOWN_VERSION;
  ...
  Bootstrap: {
    ...
    test(win) {
      const jQueryAvailable = win.$ && win.$.fn;
      const RE_PREFIX_V2 = '\\$this\\.data\\((?:\'|")';
      const RE_PREFIX_V3 = '\\$this\\.data\\((?:\'|")(?:bs\\.){1}';
      ...
      if (jQueryAvailable) {
        ...
        bootstrapComponents.some((component) => {
          if (win.$.fn[component]) {
            ...
            if (new RegExp(RE_PREFIX_V3 + component).test(win.$.fn[component].toString())) {
              bootstrapVersion = '>= 3.0.0 & <= 3.1.1';
              return true;
              // Bootstrap < 3.1.0 detection
            }
            if (new RegExp(RE_PREFIX_V2 + component).test(win.$.fn[component].toString())) {
  ...
  'jQuery UI': {
    ...
    npm: 'jquery-ui',
    test(win) {
      const jq = win.jQuery || win.$ || win.$jq || win.$j;
      if (jq && jq.fn && jq.fn.jquery && jq.ui) {
        const plugins = 'accordion,datepicker,dialog,draggable,droppable,progressbar,resizable,selectable,slider,menu,grid,tabs'.split(
          ',',
        );
        const concat = [];
  ...
  Dojo: {
    ...
    test(win) {
      if (win.dojo && win.dojo.delegate) {
        const version = win.dojo.version ? win.dojo.version.toString() : UNKNOWN_VERSION;
        return { version, details: `Details: ${win.dijit ? 'Uses Dijit' : 'none'}` };
      }
      return false;
    },
  },
  ...
  Preact: {
    ...
    test(win) {
      ...
      let preactRoot = getMatch(document.body) || getMatch(document.body.firstElementChild);
      if (!preactRoot) {
        preactRoot = document
  ...
  'Processing.js': {
    ...
    test(win) {
      if (win.Processing && win.Processing.box) {
        return { version: win.Processing.version || UNKNOWN_VERSION };
      }
      return false;
    },
  },
  ...
  Underscore: {
    ...
    npm: 'underscore',
    test(win) {
      if (win._ && typeof win._.tap === 'function' && !testRules['Lo-Dash'].test(win)) {
  ...
  MochiKit: {
    ...
    test(win) {
      if (win.MochiKit && win.MochiKit.Base.module) {
        return { version: win.MochiKit.VERSION || UNKNOWN_VERSION };
      }
      return false;
    },
  },
  ...
  'Velocity.js': {
    ...
    test(win) {
      const jq = win.jQuery || win.$;
      const velocity = jq ? jq.Velocity : win.Velocity;

      if (velocity && velocity.RegisterEffect && velocity.version) {
        return {
          version: `${velocity.version.major}.${velocity.version.minor}.${velocity.version.patch}`,
        };
      }
  ...
  Brewser: {
    ...
    test(win) {
      if (win.BREWSER && win.BREWSER.ua) {
        return { version: win.BREWSER.VERSION || UNKNOWN_VERSION };
      }
      return false;
    },
  },
  ...
  ScrollMagic: {
    ...
    test(win) {
      if (win.ScrollMagic && win.ScrollMagic.Controller) {
        return { version: win.ScrollMagic.version || UNKNOWN_VERSION };
      }
      return false;
    },
  },
  ...
  Marko: {
    ...
    npm: 'marko',
    test(win) {
      const selector = '[data-marko-key], [data-marko]';
      const markoElement = win.document.querySelector(selector);
      if (markoElement) {
        return { version: UNKNOWN_VERSION };
      }
      return false;
  ...
  AMP: {
    ...
    npm: null,
    test(win) {
      const version = win.document.documentElement.getAttribute('amp-version');
  ...
  Workbox: {
    ...
    async test(win) {
      ...
      return nav.serviceWorker
        .getRegistration()
        .then(() => {
          const { scriptURL } = nav.serviceWorker.controller;
          return fetch(scriptURL, {
            ...
            .then((scriptContent) => {
              ...
              if (workboxRegExp.test(scriptContent)) {
                ...
                const semVerRegExp = /workbox.*?\b((0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(-(0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*)(\.(0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*))*)?(\+[0-9a-zA-Z-]+(\.[0-9a-zA-Z-]+)*)?)\b/gim;
                const matches = semVerRegExp.exec(scriptContent);
                let version = UNKNOWN_VERSION;
